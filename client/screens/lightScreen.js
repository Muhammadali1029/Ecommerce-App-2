  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const lightProducts = response.data.products.filter(product => product.category === 'light');
        setProducts(lightProducts);

        // Check if there are no products in the "light" category
  ...
  const renderItem = ({ item }) => (
    ...
    >
      ...
      <View style={styles.itemDetails}>
        ...
        {item.stock > 0 && item.stock <= 10 && <Text style={styles.stockStatus}>Only a few in stock</Text>}
      </View>
    </TouchableOpacity>
  );
